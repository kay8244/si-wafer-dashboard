...
import { useNandCustomerNews, NewsArticle } from '@/hooks/useNandCustomerNews';
...
export default function NandCustomerNews({
  customerId,
  companyName,
  companyColor,
  onClose,
}: NandCustomerNewsProps) {
      ...
      {!loading && !error && articles.length > 0 && (
        <div className="space-y-4">
          {answer && (
            ...
            >
              <h3 className="mb-3 text-sm font-bold text-slate-800">AI 요약</h3>
          ...
          {articles.map((article, i) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-start gap-4 rounded-xl border border-slate-200 bg-white p-4 transition-all hover:shadow-md"
            >
              <span
              ...
              <div className="min-w-0 flex-1">
                <h4 className="mb-1 text-sm font-semibold text-slate-900 group-hover:text-blue-600">
                  {article.title}
                </h4>
