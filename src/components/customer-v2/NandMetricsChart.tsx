...
export default function NandMetricsChart({ customers, onSelectMetric }: NandMetricsChartProps) {
  ...
  return (
    <div className="space-y-10">
      ...
      <section>
        <h2
          className="mb-4 cursor-pointer text-lg font-semibold text-slate-800 transition-colors hover:text-blue-600"
          onClick={() => onSelectMetric?.('asp')}
        >
          NAND ASP 변동률 (QoQ)
        </h2>
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <p className="mb-4 text-xs text-slate-400">※ 분기별 평균판매가격 변동률(%)</p>
          <ResponsiveContainer width="100%" height={450}>
            <BarChart data={aspData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              {NAND_CUSTOMER_IDS.map((id) => (
                <Bar key={id} dataKey={id} fill={NAND_CUSTOMERS[id].color} radius={[3, 3, 0, 0]} maxBarSize={40} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <section>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <p className="mb-4 text-xs text-slate-400">※ 분기별 비트 출하량 변동률(%)</p>
          <ResponsiveContainer width="100%" height={450}>
      ...
      <section>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <p className="mb-4 text-xs text-slate-400">※ NAND 출하 대비 QLC 비중(%)</p>
          <ResponsiveContainer width="100%" height={450}>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6">
            <p className="mb-4 text-xs text-slate-400">※ 생산설비 가동률(%)</p>
        ...
        <section>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6">
            <p className="mb-4 text-xs text-slate-400">※ 평균 재고 보유일수</p>
            <ResponsiveContainer width="100%" height={300}>
