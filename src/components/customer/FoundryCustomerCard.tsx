...
import { FOUNDRY_FINANCIAL_METRICS } from '@/lib/foundry-constants';
...
export default function FoundryCustomerCard({ data, selected, onSelect }: FoundryCustomerCardProps) {
  const { customer, segmentFinancials, foundryMetrics, latestGrowth, error } = data;
  const latest = segmentFinancials[segmentFinancials.length - 1];
  const latestMetrics = foundryMetrics[foundryMetrics.length - 1];
    ...
    >
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          {latestMetrics?.advancedNodeRatio != null && (
            <span className="rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700">
              첨단노드 {latestMetrics.advancedNodeRatio}%
            </span>
          )}
          <div
      ...
      {error ? (
        ...
        <div className="space-y-3">
          ...
          {FOUNDRY_FINANCIAL_METRICS.map(({ key, labelKo }) => {
          ...
          {/* Foundry-specific metrics summary */}
          {latestMetrics && (
            <div className="mt-3 border-t pt-3">
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                {latestMetrics.waferAspChangeQoQ != null && (
                  <div>
                    <span className="text-gray-400">ASP QoQ</span>
                    <span className={`ml-1 font-semibold ${latestMetrics.waferAspChangeQoQ >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {formatPercent(latestMetrics.waferAspChangeQoQ)}
                    </span>
                  </div>
                )}
                {latestMetrics.waferShipmentGrowthQoQ != null && (
                  <div>
                    <span className="text-gray-400">출하 QoQ</span>
                    <span className={`ml-1 font-semibold ${latestMetrics.waferShipmentGrowthQoQ >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {formatPercent(latestMetrics.waferShipmentGrowthQoQ)}
                    </span>
                  </div>
                )}
              </div>
