...
import { COMPANIES, COMPANY_IDS } from '@/lib/constants';
import { formatCurrency, formatCurrencyOrNA, formatExchangeRate } from '@/lib/format';
...
export default function QuarterlyTable({ companies }: QuarterlyTableProps) {
  const [currencyMode, setCurrencyMode] = useState<CurrencyMode>('krw');

  // Get all quarters, sorted recent first
    ...
    <section className="mb-8">
            ...
            <tr className="border-b bg-gray-50/50">
              ...
              {COMPANY_IDS.map((id) => (
                <th key={id} colSpan={4} className="px-0 py-0">
                  <div className="flex">
                    <span className="flex-1 px-2 py-2 text-right text-xs font-medium text-gray-500">
                      매출
                    </span>
                    <span className="flex-1 px-2 py-2 text-right text-xs font-medium text-gray-500">
                      영업이익
                  ...
                  return (
                    <td key={id} colSpan={4} className="px-0 py-0">
                      <div className="flex">
                        ...
                        <span className="flex-1 px-2 py-3 text-right text-xs text-gray-700">
                          {q ? formatCurrencyOrNA(oi!, currency) : '-'}
                        </span>
                        <span className="flex-1 px-2 py-3 text-right text-xs text-gray-700">
                          {q ? formatCurrency(ni!, currency) : '-'}
                        </span>
                        <span className="flex-1 px-2 py-3 text-right text-xs text-gray-700">
                          {q ? formatCurrencyOrNA(ebitda!, currency) : '-'}
                        </span>
                      </div>
                    </td>
                  );
      ...
      {currencyMode === 'krw' && (
        <div className="mt-2 flex flex-wrap gap-4 text-xs text-gray-400">
          <span>적용 환율 (분기말 기준):</span>
          {COMPANY_IDS.map((id) => {
            const latest = companies[id].quarterlies[companies[id].quarterlies.length - 1];
            if (!latest) return null;
            return (
              <span key={id}>
                {COMPANIES[id].nameKo}: {formatExchangeRate(latest.exchangeRate, COMPANIES[id].currency)}
              </span>
            );
          })}
        </div>
