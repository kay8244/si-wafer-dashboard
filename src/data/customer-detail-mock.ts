import type { CustomerExecutive, CustomerDetailId, MonthlyMetricData, ConfigurableKpi, NewsCategory, AggregateCustomerId, WaferInOutQuarterlyEntry, BitGrowthQuarterlyEntry, EstimateTrendData } from '@/types/indicators';
...
function genWaferInOutQuarterly(baseIn: number, baseOut: number, dramRatio?: number): WaferInOutQuarterlyEntry[] {
  ...
  return qs.map((q, i) => {
    ...
    return {
      ...
      ...(dramRatio !== undefined ? { dramRatio: +(dramRatio + (seededValue(i * 31 + dramRatio * 100) - 0.5) * 0.04).toFixed(3) } : {}),
    };
  });
}
...
function genBitGrowthQuarterly(baseGrowth: number): BitGrowthQuarterlyEntry[] {
  ...
  return qs.map((q, i) => ({
    ...
    growthTF: +(baseGrowth * 0.95 + (seededValue(i * 17 + baseGrowth * 7) - 0.5) * 7 + i * 0.45).toFixed(1),
    isEstimate: q.includes('(E)'),
  }));
}
...
export const CUSTOMER_EXECUTIVES: Record<CustomerDetailId, CustomerExecutive> = {
  ...
  SEC_Foundry: {
    ...
    estimateTrend: genEstimateTrend(30, 28, 4.0),
    scrapRate: [
