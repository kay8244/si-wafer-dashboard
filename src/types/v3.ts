...
export type LeadingIndicatorRating = '상' | '중상' | '중' | '중하' | '하';
...
export type DeviceType = 'dram' | 'hbm' | 'nand' | 'otherMemory' | 'foundry' | 'logic' | 'analog' | 'discrete' | 'sensor';
...
export interface TotalWaferQuarterlyEntry {
  quarter: string;
  total: number;
  pw: number;
  epi: number;
...
export type AppCategoryType = 'server' | 'smartphone' | 'pc' | 'automotive' | 'industrial';

export interface AppCategoryItem {
  category: AppCategoryType;
  label: string;
  checked: boolean;
  subTypes: ApplicationType[];
}
...
export interface WaferInOutQuarterlyEntry {
  quarter: string;
  waferIn: number;
  waferOut: number;
  isEstimate: boolean;
