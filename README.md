...
`tests/fixtures/*-mock.ts` 파일들은 **seed 스크립트에서만** 사용되며, 컴포넌트가 직접 import하지 않습니다.
...
```bash
# SQLite (로컬)
npm run seed                   # tests/fixtures/*-mock.ts -> data/dashboard.db (~18,141 rows, VCM 2버전 포함)
...
# tests/fixtures/*-mock.ts의 데이터를 실제 값으로 교체 후:
npm run seed                   # SQLite
...
|
+-- tests/
|   +-- fixtures/                  # Mock 데이터 (seed 스크립트 전용, 프로덕션 번들 미포함)
